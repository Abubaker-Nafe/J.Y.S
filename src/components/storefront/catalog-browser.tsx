...
export function CatalogBrowser({
  ...
  const hasFilters = Boolean(query || filters.category !== defaultCategory || filters.available || filters.sort !== "featured");
  ...
  function clearFilters() {
    ...
    setFilterDraft({ baseline: serverFilterKey, category: defaultCategory, available: false, sort: "featured" });
  ...
  function renderFilters(scope: "desktop" | "mobile") {
    ...
    return <>
      ...
      <div><Label htmlFor={sortId}>{t("catalog.sort")}</Label><Select id={sortId} value={filters.sort} disabled={isPending} onChange={(event) => { const sort = event.target.value as StorefrontSort; setFilterDraft({ ...filters, baseline: serverFilterKey, sort }); navigate({ sort, page: 1 }); }}><option value="featured">{t("catalog.featured")}</option><option value="newest">{t("catalog.newest")}</option><option value="low">{t("catalog.priceLow")}</option><option value="high">{t("catalog.priceHigh")}</option></Select></div>
