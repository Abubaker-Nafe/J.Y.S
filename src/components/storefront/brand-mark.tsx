...
export function BrandMark({ locale, inverted = false }: { locale: Locale; inverted?: boolean }) {
  return (
    <Link href={`/${locale}`} className="group inline-flex items-center gap-3" aria-label={locale === "ar" ? "الصفحة الرئيسية لـ JYS" : "JYS home"}>
      ...
      <span className="flex flex-col leading-none"><strong className={`text-xl font-black tracking-[0.12em] ${inverted ? "text-white" : "text-ink"}`}>JYS</strong><small className={`mt-1 text-[10px] font-bold uppercase tracking-[.12em] ${inverted ? "text-white/80" : "text-muted"}`}>Barber supply</small></span>
    </Link>
  );
}
