    ...
    <header className="sticky top-0 z-50 border-b border-line/80 bg-canvas/95 backdrop-blur-xl" data-no-print>
      <div className="container-shell flex h-[74px] items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <details ref={mobileMenu} className="group relative lg:hidden"><summary className="grid size-11 cursor-pointer list-none place-items-center rounded-full hover:bg-ink/6" aria-label={t("nav.menu")}><Menu className="size-5" /></summary><div className="absolute end-0 top-[calc(100%+1rem)] w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-line bg-surface-strong p-3 shadow-lift"><form action={`/${locale}/search`} onSubmit={closeMobileMenu} className="relative mb-2"><label className="sr-only" htmlFor="mobile-search">{t("nav.search")}</label><input id="mobile-search" name="q" type="search" className="h-11 w-full rounded-xl border bg-canvas ps-10 pe-3 text-sm" placeholder={t("nav.search")} /><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted" /></form>{[["nav.home", ""], ["nav.products", "/products"], ["nav.delivery", "/delivery"], ["nav.pickup", "/pickup"]].map(([label, path]) => <Link key={label} href={`/${locale}${path}`} onClick={closeMobileMenu} className="block rounded-xl px-4 py-3 text-sm font-bold hover:bg-canvas">{t(label ?? "")}</Link>)}<div className="my-2 border-t border-line" /><Link href={`/${locale}/categories`} onClick={closeMobileMenu} className="block rounded-xl px-4 py-3 text-sm font-black text-accent hover:bg-canvas">{locale === "ar" ? "كل التصنيفات" : "All categories"}</Link><p className="px-4 py-2 text-xs font-black uppercase tracking-wider text-muted">{t("nav.categories")}</p><div className="grid grid-cols-2">{categories.map((category) => <Link key={category.id} href={`/${locale}/category/${category.slug}`} onClick={closeMobileMenu} className="rounded-lg px-4 py-2 text-sm hover:bg-canvas">{localize(category.name, locale)}</Link>)}</div><div className="mt-3 px-3 sm:hidden"><LanguageSwitcher locale={locale} /></div></div></details>
        </div>
      </div>
    </header>
  </>;
