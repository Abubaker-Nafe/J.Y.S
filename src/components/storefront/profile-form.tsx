...
export function ProfileForm({ locale }: { locale: Locale }) {
  ...
  return <section className="rounded-3xl border border-line bg-surface-strong p-6 shadow-soft md:p-9">
    <div className="flex min-w-0 items-center gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-brand-strong text-white"><UserRound className="size-5" /></span><div className="min-w-0"><h2 className="break-anywhere font-display text-2xl font-semibold">{translate(locale, "account.overview")}</h2><p className="break-anywhere text-sm text-muted">{translate(locale, "account.profileText")}</p></div></div>
