...
function mapOrderSummary(order: {
  ...
  return {
    ...
    total: safeNumber(order.subtotal),
...
export async function getDashboardData(): Promise<AdminDashboardData> {
  ...
  const [salesAggregate, todayCount, weekCount, monthCount, newOrders, stockProducts, recentOrders, statusGroups, chartOrders] = await Promise.all([
    db.order.aggregate({ where: { status: { in: [...fulfilledStatuses] } }, _sum: { subtotal: true }, _avg: { subtotal: true } }),
    ...
    db.order.findMany({ orderBy: { createdAt: "desc" }, take: 8, select: { id: true, orderNumber: true, customerName: true, customerEmail: true, customerPhone: true, status: true, paymentStatus: true, fulfillmentMethod: true, currency: true, subtotal: true, createdAt: true } }),
    db.order.groupBy({ by: ["status"], _count: { _all: true } }),
    db.order.findMany({ where: { createdAt: { gte: chartStart }, status: { in: [...fulfilledStatuses] } }, select: { createdAt: true, subtotal: true } }),
  ]);
  ...
  for (const order of chartOrders) {
    ...
    if (item) {
      item.revenue += safeNumber(order.subtotal);
      item.orders += 1;
    }
  }
  ...
  return {
    metrics: {
      totalSales: safeNumber(salesAggregate._sum.subtotal),
      ordersToday: todayCount,
      ordersWeek: weekCount,
      ordersMonth: monthCount,
      averageOrderValue: safeNumber(salesAggregate._avg.subtotal),
      newOrders,
...
export async function listProducts(input: { page?: number; search?: string; status?: string; categoryId?: string; availability?: string; stockState?: string; saleState?: string }): Promise<Paginated<AdminProductSummary>> {
  ...
  const saleState = input.saleState && ["ALL", "ACTIVE", "SCHEDULED", "EXPIRED", "DISABLED"].includes(input.saleState) ? input.saleState : "ALL";
  const where: Prisma.ProductWhereInput = {
    ...
    ...(search ? { OR: [{ sku: { contains: search, mode: "insensitive" } }, { nameAr: { contains: search, mode: "insensitive" } }, { nameEn: { contains: search, mode: "insensitive" } }] } : {}),
  };
  const postFilterStock = stockState === "LOW";
  const postFilterSale = saleState !== "ALL";
  const [rows, databaseTotal] = await Promise.all([
    db.product.findMany({ where, orderBy: { updatedAt: "desc" }, ...(postFilterStock || postFilterSale ? {} : { skip: (page - 1) * pageSize, take: pageSize }), include: { category: { select: { nameAr: true, nameEn: true } }, images: { where: { isPrimary: true }, take: 1, select: { url: true } }, variants: { select: { stockQuantity: true } } } }),
    db.product.count({ where }),
  ]);
  const mapped = rows
    .map((product) => {
      const pricing = resolveSalePricing({ normalPrice: product.price, isOnSale: product.isOnSale, salePrice: product.salePrice, saleStartsAt: product.saleStartsAt, saleEndsAt: product.saleEndsAt, productActive: product.status === "ACTIVE" && product.isAvailable, archived: Boolean(product.archivedAt) });
      return { id: product.id, sku: product.sku, nameAr: product.nameAr, nameEn: product.nameEn, price: safeNumber(product.price), effectivePrice: Number(pricing.effectivePrice), saleEnabled: product.isOnSale, salePrice: product.salePrice === null ? null : safeNumber(product.salePrice), saleStartsAt: product.saleStartsAt?.toISOString() ?? null, saleEndsAt: product.saleEndsAt?.toISOString() ?? null, saleUpdatedAt: product.saleUpdatedAt?.toISOString() ?? null, saleStatus: pricing.status, discountPercentage: pricing.discountPercentage, stock: product.variants.length ? product.variants.reduce((sum, variant) => sum + variant.stockQuantity, 0) : product.stockQuantity, lowStockThreshold: product.lowStockThreshold, status: product.status, available: product.isAvailable, active: product.status === "ACTIVE", featured: product.isFeatured, variationCount: product.variants.length, archivedAt: product.archivedAt?.toISOString() ?? null, categoryNameAr: product.category.nameAr, categoryNameEn: product.category.nameEn, primaryImageUrl: product.images[0]?.url ?? null, updatedAt: product.updatedAt.toISOString() } satisfies AdminProductSummary;
    })
    .filter((product) => (!postFilterStock || (product.stock > 0 && product.stock <= product.lowStockThreshold)) && (!postFilterSale || product.saleStatus === saleState));
  const total = postFilterStock || postFilterSale ? mapped.length : databaseTotal;
  return {
    items: postFilterStock || postFilterSale ? mapped.slice((page - 1) * pageSize, page * pageSize) : mapped,
    page,
    pageSize,
    total,
...
export async function getProduct(id: string): Promise<AdminProductDetail | null> {
  ...
  return {
    ...
    saleEnabled: product.isOnSale,
    salePrice: product.salePrice === null ? null : safeNumber(product.salePrice),
    saleStartsAt: product.saleStartsAt?.toISOString() ?? null,
    saleEndsAt: product.saleEndsAt?.toISOString() ?? null,
    saleUpdatedAt: product.saleUpdatedAt?.toISOString() ?? null,
    saleStatus: pricing.status,
...
export async function listOrders(input: { page?: number; search?: string; status?: string; paymentStatus?: string; fulfillment?: string; from?: string; to?: string }): Promise<Paginated<AdminOrderSummary>> {
  ...
  const select = { id: true, orderNumber: true, customerName: true, customerEmail: true, customerPhone: true, status: true, paymentStatus: true, fulfillmentMethod: true, currency: true, subtotal: true, createdAt: true } as const;
...
export async function getOrder(id: string): Promise<AdminOrderDetail | null> {
  const order = await db.order.findUnique({ where: { id }, include: { items: { include: { product: { select: { images: { orderBy: [{ isPrimary: "desc" }, { displayOrder: "asc" }], take: 1, select: { url: true, altAr: true, altEn: true } } } } } }, statusHistory: { orderBy: { createdAt: "desc" }, include: { changedBy: { select: { name: true, email: true } } } } } });
  ...
  return {
    ...
    items: order.items.map((item) => ({ id: item.id, productId: item.productId, sku: item.skuSnapshot, productNameAr: item.productNameAr, productNameEn: item.productNameEn, variantLabelAr: item.variantLabelAr, variantLabelEn: item.variantLabelEn, unitPrice: safeNumber(item.unitPrice), quantity: item.quantity, lineTotal: safeNumber(item.lineTotal), imageUrl: item.imageUrlSnapshot ?? item.product?.images[0]?.url ?? null, imageAltAr: item.imageAltArSnapshot ?? item.product?.images[0]?.altAr ?? item.productNameAr, imageAltEn: item.imageAltEnSnapshot ?? item.product?.images[0]?.altEn ?? item.productNameEn })),
...
export async function listLocations(): Promise<AdminCity[]> {
  ...
  return cities.map((city) => ({ id: city.id, slug: city.slug, nameAr: city.nameAr, nameEn: city.nameEn, active: city.isActive, displayOrder: city.displayOrder, areas: city.areas.map((area) => ({ id: area.id, slug: area.slug, nameAr: area.nameAr, nameEn: area.nameEn, active: area.isActive, displayOrder: area.displayOrder })) }));
}
