...
const report: AdminReportData = {
  ...
  orders: [{ orderNumber: "JYS-1", createdAt: "2026-07-01T10:00:00.000Z", customerName: "عميل", customerEmail: "customer@example.com", customerPhone: "+970500000000", fulfillmentMethod: "DELIVERY", paymentMethod: "CASH_ON_DELIVERY", paymentStatus: "PAID", status: "DELIVERED", currency: "ILS", subtotal: 10, total: 10, itemCount: 1 }],
  categories: [],
  ...
  insights: [],
  products: [{ id: "1", sku: "+FORMULA", nameAr: "=صيغة", nameEn: "=formula", units: 1, revenue: 12, views: 3, wishlists: 2, cartAdds: 1, stock: 4, lowStockThreshold: 5, categoryAr: "حلاقة", categoryEn: "Shaving", active: true, available: true, normalPrice: 15, salePrice: 12, effectivePrice: 12, discountPercentage: 20, saleStatus: "ACTIVE" }],
};
...
describe("report CSV exports", () => {
  ...
  it("exports product revenue without a delivery-fee column", () => {
    const output = reportCsv(report, "en", "orders");
    expect(output).not.toContain("Delivery fee");
    expect(output).toContain('"Product total"');
  });
});
