...
export default async function LocaleLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return <div lang={locale} dir={localeDirection(locale)} className="min-h-[100dvh] min-w-0 w-full"><a href="#main-content" className="fixed start-4 top-3 z-[100] -translate-y-24 rounded-full bg-accent px-5 py-3 font-bold text-white transition focus:translate-y-0">{translate(locale, "a11y.skip")}</a><StoreProvider locale={locale} initialCurrency={business?.currency ?? "ILS"}><Suspense fallback={null}><SiteHeader locale={locale} categories={catalog.categories} /></Suspense><main id="main-content" className="min-w-0" tabIndex={-1}>{children}</main><SiteFooter locale={locale} business={business} /></StoreProvider></div>;
}
