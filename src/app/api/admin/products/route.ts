...
export async function GET(request: Request) {
  try {
    ...
    const data = await listProducts({ page: Number(query.get("page") || 1), search: query.get("search") ?? undefined, status: query.get("status") ?? undefined, categoryId: query.get("category") ?? undefined });
    return adminOk(data);
