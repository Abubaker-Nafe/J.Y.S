...
import { acknowledgeCartPrices, addCartItem, getCart } from "@/lib/domain/cart-service";
...
export async function PATCH(request: Request) {
  try {
    assertSameOrigin(request);
    const session = await requireUser();
    return NextResponse.json({ cart: await acknowledgeCartPrices(session.user.id) });
  } catch (error) {
    return jsonError(error);
  }
}
